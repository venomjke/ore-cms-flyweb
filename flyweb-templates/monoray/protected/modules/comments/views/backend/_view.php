<div class="comment">
	<div class="name">
		<?php
			...
			echo ' '.CHtml::link($data->apartment->getStrByLang('title'), $data->apartment->getUrl()); ?>
	</div>
	...
	<div class="date">
		<?php if($data->status==Comment::STATUS_PENDING){ ?>
			...
			echo ' | ';
			echo CHtml::linkButton(Yii::t('module_comments', 'Delete'),
					array(
						'submit'=>array('/comments/backend/main/delete','id'=>$data->id),
						'confirm'=>Yii::t('module_comments', 'Are you sure to delete comment #{id}?', array('{id}'=>$data->id)),
					)
				);
